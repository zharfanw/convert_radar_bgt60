function [means,diffs,meanDiff,CR,linFit] = BlandAltman(var1, var2, flag, flag2)
    ...
    if flag ~= 0
        figure
        plot(means,diffs,'.','MarkerSize',10)
        hold on
        %%%xlabel('Mean of method A and B');
        xlabel('Mean of two measures (bpm)','fontweight','bold','fontsize',12);
        xlim([55 105])
        if flag2
            %%%ylabel('(method A - method B)/Average %');
            ylabel({'Difference of two measures/';'Mean of two measures (%)'},'fontweight','bold','fontsize',12);
        else
            %%%ylabel('method A - method B');
            ylabel('Difference of two measures (bpm)','fontweight','bold','fontsize',12);
        end
        ylim([-80 80])
        if flag > 1
            plot(means, ones(1,length(means)).*CR(1),':'); %%%plot the upper CR
            plot(means, ones(1,length(means)).*CR(2),':'); %%%plot the lower CR
        ...
        legend('Measurement points','+1.96 SD','-1.96 SD', 'Mean','fontweight','bold','fontsize',12)
    end
