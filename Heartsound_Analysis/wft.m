...
if ~strcmpi(PlotMode,'off')
    ...
    YY=60*freq; XX=(0:(L-1))/fs; ZZ=abs(WFT); ZZname='WFT amplitude';
    ...
    if isempty(strfind(lower(PlotMode),'wr')) && (size(ZZ,1)>MYL || size(ZZ,2)>MXL)
        ...
        if size(ZZ,1)>MYL, YY=linspace(60*freq(1),60*freq(end),MYL); end
        if size(ZZ,2)>MXL, XX=linspace(0,(L-1)/fs,MXL); end
        ZZ=aminterp((0:(L-1))/fs,60*freq,ZZ,XX,YY,'max'); XX=XX(:); YY=YY(:);
    end
    TL=length(XX); FL=length(YY);
    pc=pcolor(YY,XX,ZZ'); title(ZZname);
    
    xlabel('Rate (bpm)'); ylabel('Time (s)');
    ylim([0,(L-1)/fs]); xlim([60*freq(1),60*freq(end)]); set(pc,'EdgeColor','none');
...
%     plot([freq(1);freq(end)],[coib1;coib1]/fs,'-k','LineWidth',2); plot([freq(1);freq(end)],[L-coib2+1;L-coib2+1]/fs,'-k','LineWidth',2);
    ...
    if ~isempty(strfind(lower(PlotMode),'+'))
        axes('Position',[0.15,0.7,0.8,0.25],'Layer','top','XLim',[60*freq(1),60*freq(end)],'XTickLabel',{},'Box','on','FontSize',16);
        hold all;
